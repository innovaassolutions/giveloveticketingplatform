...
export default function LadyGagaArtistPortal() {
  const { getArtistPricing, updateArtistPricing } = usePricing();
  const artistSlug = 'lady-gaga';

  const [totalRevenue, setTotalRevenue] = useState(125000);
  const [ticketsSold, setTicketsSold] = useState(834);

  // Get current pricing from context
  const currentPricing = getArtistPricing(artistSlug);
  const { basePrice, upliftPercentage, finalPrice, charityAmount } = currentPricing;

  // Update pricing when slider changes
  const handleUpliftChange = (newUpliftPercentage: number) => {
    const newFinalPrice = basePrice * (1 + newUpliftPercentage / 100);
    const newCharityAmount = newFinalPrice - basePrice;

    updateArtistPricing(artistSlug, {
      basePrice,
      upliftPercentage: newUpliftPercentage,
            ...
            >
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  onChange={(e) => handleUpliftChange(parseFloat(e.target.value))}
